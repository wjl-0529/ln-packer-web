...
import 'package:ln_packer_web/light_novel/bili_novel/bili_novel_secret.dart';
...
class BiliNovelSource implements LightNovelSource {
  ...
  static final String _rateMode =
      Platform.environment["PACKER_SOURCE_RATE_MODE"] == "fast"
          ? "fast"
          : "stable";
  final PackRunContext? runContext;
  late final Scheduler _scheduler;
  late final Scheduler _imageScheduler;
  ...
  BiliNovelSource({this.runContext}) {
    _scheduler = runContext?.createScheduler(
          _envInt(
            "PACKER_BILI_REQUESTS_PER_MINUTE",
            _rateMode == "fast" ? 30 : 15,
          ),
          Duration(minutes: 1),
        ) ??
        Scheduler(
    ...
    _imageScheduler = runContext?.createScheduler(
          _envInt(
            "PACKER_BILI_IMAGES_PER_SECOND",
            _rateMode == "fast" ? 20 : 10,
          ),
          Duration(seconds: 1),
        ) ??
        Scheduler(
  ...
  static int _envInt(String name, int fallback) {
  ...
  Future<String> _httpGetString(String url) {
    return _scheduler.run((c) async {
      runContext?.throwIfCanceled();
      String html = await httpGetString(url,
          headers: {
      ...
      if (html.contains("Cloudflare to restrict access") ||
        ...
        c.pause();
        await (runContext?.delay(Duration(seconds: 10)) ??
